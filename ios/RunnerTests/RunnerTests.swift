...
final class RunnerTests: XCTestCase {
  ...
  func testGalleryUIDiagnosticsAreMirroredToFastDeviceLog() throws {
    let sourceURL = URL(fileURLWithPath: #filePath)
      .deletingLastPathComponent()
      .deletingLastPathComponent()
      .appendingPathComponent("Runner/NativeConnectViewController.swift")
    ...
    XCTAssertTrue(body.contains("CameraVendorGalleryDiagnostics.log(\"UI: \\(message)\""))
  ...
  func testCameraVendorPhotoDataDownloadDoesNotUseFilePreparationCommands() throws {
    XCTAssertFalse(
    ...
    XCTAssertFalse(body.contains("EventsList before data download"))
    XCTAssertFalse(body.contains("CompressionCutOff/PartialSize"))
  }
  ...
  func testRealtimeGalleryServicePublishesFastInitialHandlesBeforeFullObjectInfoFallback() throws {
    let sourceURL = URL(fileURLWithPath: #filePath)
      ...
      .appendingPathComponent("Runner/CameraVendorBluetoothService.swift")
    let source = try String(contentsOf: sourceURL, encoding: .utf8)
    let fastInitialStart = try XCTUnwrap(source.range(of: "let fastInitialInfos = try session.fastInitialGalleryObjectInfos()")?.lowerBound)
    let fallbackStart = try XCTUnwrap(source.range(of: "let infos = try session.galleryObjectInfos()", range: fastInitialStart..<source.endIndex)?.lowerBound)
    ...
    XCTAssertTrue(fastInitialRegion.contains("return fastInitialItems"))
  ...
  func testPtpPacketLevelDiagnosticsAreSuppressedDuringDownloads() {
    ...
    XCTAssertTrue(CameraVendorPtpDiagnosticLogPolicy.shouldEmit("[OBS] PTP_STANDARD_PARTIAL_OBJECT_CHUNK purpose=download-data handle=0x000004CA chunkBytes=4194304 totalBytes=4194304 chunkMs=1730 isJpeg=true"))
  }
