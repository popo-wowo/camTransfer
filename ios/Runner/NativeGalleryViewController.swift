...
final class NativeGalleryViewController: UIViewController, UIGestureRecognizerDelegate {
  ...
  private var hdPreviewPipelineInitialized = false
  private lazy var hdPreviewPipeline: CameraGalleryHDPreviewPipeline = {
    hdPreviewPipelineInitialized = true
    return CameraGalleryHDPreviewPipeline(
      cache: hdPreviewCache,
      suspendThumbnailPipeline: { [weak self] in
        await self?.runtime.suspendGalleryChildWorkForHighDefinitionPreview()
      },
      resumeThumbnailPipeline: { [weak self] in
        await self?.runtime.resumeGalleryChildWorkAfterHighDefinitionPreview()
      },
      fetchPreview: { [weak self] identity in
        guard let self else { throw CancellationError() }
        do {
          return try await self.runtime.requestPreviewImageWithInfo(for: identity.handle)
        } catch {
  ...
  private let hdDateButton: UIButton = {
    let button = UIButton(type: .system)
    button.translatesAutoresizingMaskIntoConstraints = false
    var configuration = UIButton.Configuration.filled()
    configuration.title = "选择日期"
    ...
    button.accessibilityLabel = "选择高清预览日期"
    return button
  }()

  private lazy var hdTopChipRow: UIStackView = {
    let arrangedSubviews: [UIView]
    if NativeGalleryHDChromePolicy.showsLoadCountBeforeDate {
      arrangedSubviews = [hdStatusLabel, hdDateButton]
    } else {
      arrangedSubviews = [hdDateButton, hdStatusLabel]
    }
  ...
  deinit {
    ...
    let hdPreviewPipeline: CameraGalleryHDPreviewPipeline? = hdPreviewPipelineInitialized ? self.hdPreviewPipeline : nil
    let hdPreviewCache = hdPreviewCache
    Task { @MainActor in
      visibleThumbnailRefreshTask?.cancel()
      await hdTransitionTask?.value
      if let hdPreviewPipeline {
  ...
  private func setupUI() {
    ...
    hdDateButton.addTarget(self, action: #selector(selectHDDateTapped), for: .touchUpInside)
    ...
    NSLayoutConstraint.activate([
      ...
      hdDateButton.heightAnchor.constraint(equalToConstant: 30),
  ...
  private func switchBrowseMode(_ mode: NativeGalleryBrowseMode) {
    ...
    case .thumbnail:
      ...
      enqueueHDTransition { [weak self] in
        await self?.hdPreviewPipeline.deactivate(resumeThumbnailPipeline: true)
  ...
  private func startHDPreviewLoading() {
    ...
    applyHDPreviewState(NativeGalleryHDPreviewState(
      snapshot: snapshot,
      loadedHandles: hdPreviewCache.loadedHandles(for: catalogIdentity)
    ))
    let visibleHandles = hdVisibleHandles()
    enqueueHDTransition { [weak self] in
      guard let self else { return }
      await self.hdPreviewPipeline.activate(
  ...
  private func updateHDStatusLabel(_ state: NativeGalleryHDPreviewState?) {
    ...
    formatter.dateFormat = NativeGalleryHDChromePolicy.dateFormat
    hdDateButton.configuration?.title = formatter.string(from: state.snapshot.activeDate)
    hdDateButton.isHidden = false
    hdStatusLabel.text = "  \(state.loadedCount)/\(state.totalCount)  "
    hdStatusLabel.isHidden = false
    hdTopChipRow.isHidden = false
  }

  @objc private func selectHDDateTapped() {
    guard browseMode == .highDefinition,
          NativeGalleryDownloadModePresentationPolicy.canInteractWithGallery(isDownloading: runtime.isDownloading) else {
      return
    }
    let dates = NativeGalleryHDPreviewSessionPolicy.availableDates(items: catalogPresentation.items)
    guard !dates.isEmpty else {
      showToast("没有可选择的拍摄日期")
      return
    }
    let formatter = DateFormatter()
    formatter.locale = Locale(identifier: "zh_CN")
    formatter.dateFormat = NativeGalleryHDChromePolicy.dateFormat
    let alert = UIAlertController(title: "选择高清预览日期", message: nil, preferredStyle: .actionSheet)
    dates.forEach { [weak self] date in
      alert.addAction(UIAlertAction(title: formatter.string(from: date), style: .default) { _ in
        guard let self else { return }
        self.hdActiveDate = date
        self.startHDPreviewLoading()
      })
    }
    alert.addAction(UIAlertAction(title: "取消", style: .cancel))
    if let popover = alert.popoverPresentationController {
      popover.sourceView = hdDateButton
      popover.sourceRect = hdDateButton.bounds
    }
    present(alert, animated: true)
  }
  ...
  private func hdLoadState(for item: CameraVendorGalleryItem) -> NativeGalleryHDPreviewCell.LoadState {
    ...
    if let identity = hdMediaIdentity(for: handle),
       let data = hdPreviewCache.restoreLoadedData(for: identity) {
      if let image = CameraVendorGalleryThumbnailRenderer.decoded(
        from: data,
        objectOrientation: hdPreviewCache.restoreLoadedPreview(for: identity)?.objectOrientation
      ) {
        return .loaded(image)
      }
      return .failed
  ...
  private func configureHDCell(
    ...
    cell.onQueueTapped = { [weak self] in
      ...
      if case .failed = state {
        self.hdPreviewPipeline.retry(handle: handle)
        return
      }
  ...
  private func openDownloadCenter(for handles: [Int]) {
    ...
    if browseMode == .highDefinition {
      enqueueHDTransition { [weak self] in
        guard let self else { return }
        await self.hdPreviewPipeline.pauseForDownload()
        startDownload()
      }
      return
    }
...
extension NativeGalleryViewController {
  ...
  private func presentPreview(startingAt index: Int) {
    ...
    let controller = NativePhotoPreviewViewController(
      ...
      initialIndex: index,
      runtime: runtime,
      previewImageCache: hdPreviewCache,
      previewCatalogIdentity: runtime.galleryCatalogIdentity,
...
extension NativeGalleryViewController: UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(
    ...
    if collectionView === hdCollectionView {
      ...
      if let handle,
         let identity = hdMediaIdentity(for: handle),
         let data = hdPreviewCache.restoreLoadedData(for: identity),
  ...
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    hdPreviewPipeline.updateVisibleHandles(visibleHandles)
  }
  ...
  private func hdScrollDidSettle() {
    guard browseMode == .highDefinition else { return }
    hdPreviewPipeline.updateVisibleHandles(hdVisibleHandles())
  }

  private func hdMediaIdentity(for handle: Int) -> CameraGalleryMediaIdentity? {
    guard let catalogIdentity = runtime.galleryCatalogIdentity else { return nil }
