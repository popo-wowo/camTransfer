...
enum CameraVendorDownloadPipelinePolicy {
  ...
  static func shouldUseDataFastPath(
    ...
    guard mediaType == .photo else { return false }
    guard let compressedSize else { return true }
    return compressedSize < streamDownloadThresholdBytes
  }
}
...
final class CameraVendorRealtimeGalleryService: CameraGallerySession, CameraVendorGalleryBackgroundKeepAlive, CameraVendorPriorityDownloadPreparing, CameraVendorVisibleThumbnailLaneCoordinating {
  ...
  func fetchGallery() async throws -> [CameraVendorGalleryItem] {
    ...
    if directPTPMode {
      report("直接 PTP 模式已禁用：必须先完成 BLE 授权和官方 Wi-Fi 凭据确认")
      throw NSError(
        domain: "CameraVendorRealtimeGalleryService",
        code: 8,
        userInfo: [
          NSLocalizedDescriptionKey: "请先从已配对相机进入相册，完成蓝牙授权和相机 Wi-Fi 凭据确认。"
        ]
      )
    }

    guard !wifiConfigurations.isEmpty else {
      report("没有本次 BLE 返回的官方相机 Wi-Fi 配置，停止进入 PTP")
      throw NSError(
        domain: "CameraVendorRealtimeGalleryService",
        code: 9,
        userInfo: [
          NSLocalizedDescriptionKey: "相机没有返回本次官方 Wi-Fi 名称和密码，请重新从已配对相机进入相册。"
        ]
      )
    }
    ...
    try ensureCommunicationGenerationIsCurrent(fetchGeneration)
  ...
  private func fetchGallerySync(
    ...
    do {
      ...
      let fastInitialInfos = try session.fastInitialGalleryObjectInfos()
        .filter { $0.handle != 0 }
      let fastInitialFormatHints = session.cameraVendorInitialPlaceholderFormatHintsByHandle()
